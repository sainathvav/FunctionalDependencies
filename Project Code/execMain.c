...
extern int store_rel_metadata;
extern int current_relid;
extern int return_only_one;
extern int current_rel_natts;
extern relationAttributeInfoData* current_rel_attribute_info;
extern fdInfoData* current_rel_fd_info;
extern int store_attribute_data;
extern int current_att_num;
extern int get_num_fds;
extern int num_fds;
extern int get_fds;
extern int current_fd_num;
extern int check_fd;
extern int check_violated;
					 ...
					 ScanDirection direction, uint64 count, bool execute_once)
{
	...
	if (store_rel_metadata || store_attribute_data || get_num_fds || get_fds || check_fd) {
		sendTuples = 0;
	}
	else {
		sendTuples = (operation == CMD_SELECT ||
			...
			bool execute_once)
{
	...
	for (;;)
	{
		...
		for (int i = 0; i < slot -> tts_tupleDescriptor -> natts; i++) {
			...
			if (store_rel_metadata == 2 && current_relid == 0) {
				current_relid = DatumGetInt32(attr);
				store_rel_metadata = 1;
			}
			else if (store_rel_metadata == 1 && current_rel_natts == 0) {
				current_rel_natts = DatumGetInt32(attr);
				store_rel_metadata = 0;
			}
			if (store_attribute_data) {
				if (i == 0) {
					strcpy(current_rel_attribute_info[current_att_num]->attname, DatumGetCString(attr));
				}
				else if (i == 1) {
					current_rel_attribute_info[current_att_num]->attlen = DatumGetInt16(attr);
				}
				else if (i == 2) {
					current_rel_attribute_info[current_att_num]->attnum = DatumGetInt16(attr);
				}
				else if (i == 3) {
					current_rel_attribute_info[current_att_num]->attndims = DatumGetInt32(attr);
				}
				else if (i == 4) {
					current_rel_attribute_info[current_att_num]->attbyval = DatumGetBool(attr);
				}
			}
			if (get_num_fds) {
				num_fds = DatumGetInt32(attr);
			}
			if (get_fds == 1) {		/* Pass 1 */
				if (i == 0) {
					current_rel_fd_info[current_fd_num]->relname[0] = '\0';
					char dup_relname_str[65];
					strcpy(dup_relname_str, DatumGetCString(attr));
					strncpy(current_rel_fd_info[current_fd_num]->relname, &dup_relname_str[1], strlen(dup_relname_str) - 1);
				}
				else if (i == 1) {
					current_rel_fd_info[current_fd_num]->relid = DatumGetInt32(attr);
				}
				else if (i == 2) {
					char dup_base_atts_string[100];
					dup_base_atts_string[0] = '\0';
					strcpy(dup_base_atts_string, DatumGetCString(attr));
					// dup_base_atts_string = (char*) attr;
					char base_atts_string[100];
					strncpy(base_atts_string, &dup_base_atts_string[1], strlen(dup_base_atts_string) - 1);
					int j = 0;		/* Index for parsing the base atrribute string */
					int curr_base_attr = 0;
					char base_attr_str[15];
					int base_attr_ind = 0;
					while (base_atts_string[j] != 'a') {
						if (base_atts_string[j] == ',') {
							base_attr_str[base_attr_ind] = '\0';
							current_rel_fd_info[current_fd_num]->base_atts[curr_base_attr] = atoi(base_attr_str);
							curr_base_attr += 1;
							base_attr_ind = 0;
							j += 2;
							continue;
						}
						base_attr_str[base_attr_ind] = base_atts_string[j];
						base_attr_ind += 1;
						j++;
					}
					base_attr_str[base_attr_ind] = '\0';
					...
					current_rel_fd_info[current_fd_num]->base_count = DatumGetInt32(attr);
					// current_rel_fd_info[current_fd_num]->base_atts = (int *) palloc(current_rel_fd_info[current_fd_num]->base_count * sizeof(int));
				}
				else if (i == 4) {
					char dup_derived_atts_string[100];
					dup_derived_atts_string[0] = '\0';
					strcpy(dup_derived_atts_string, DatumGetCString(attr));
					// dup_base_atts_string = (char*) attr;
					char derived_atts_string[100];
					strncpy(derived_atts_string, &dup_derived_atts_string[1], strlen(dup_derived_atts_string) - 1);
					int j = 0;		/* Index for parsing the base atrribute string */
					int curr_derived_attr = 0;
					char derived_attr_str[15];
					int derived_attr_ind = 0;
					while (derived_atts_string[j] != 'a') {
						if (derived_atts_string[j] == ',') {
							derived_attr_str[derived_attr_ind] = '\0';
							current_rel_fd_info[current_fd_num]->derived_atts[curr_derived_attr] = atoi(derived_attr_str);
							curr_derived_attr += 1;
							derived_attr_ind = 0;
							j += 2;
							continue;
						}
						derived_attr_str[derived_attr_ind] = derived_atts_string[j];
						derived_attr_ind += 1;
						j++;
					}
					derived_attr_str[derived_attr_ind] = '\0';
					...
					current_rel_fd_info[current_fd_num]->derived_count = DatumGetInt32(attr);
					// current_rel_fd_info[current_fd_num]->derived_atts = (int *) palloc(current_rel_fd_info[current_fd_num]->derived_count * sizeof(int));
				}
			}
		}
			// Datum val;
