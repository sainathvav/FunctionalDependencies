 ...
 * ----------------
 */
int store_rel_metadata = 0;
int return_only_one = 0;
int current_relid = 0;
int current_rel_natts = 0;
int store_attribute_data = 0;
int current_att_num = 0;
int get_num_fds = 0;
int num_fds = 0;
int get_fds = 0;
int current_fd_num = 0;
int check_violated = 0;
int check_fd = 0;

relationAttributeInfoData* current_rel_attribute_info;
fdInfoData* current_rel_fd_info;
...
int getAttNumFromCurrRelation (const char* attname, int natts, relationAttributeInfoData* attribute_info) {
	for (int i = 0; i < natts; i++) {
		if (strcmp(attname, attribute_info[i]->attname) == 0) {
			return (i + 1);
		}
	}
	return -1;
...
char* getAttName (int attnum, int natts, relationAttributeInfoData* attribute_info) {
	char* attribute_name = palloc(65*sizeof(char)); 
	for (int i = 0; i < natts; i++) {
		if (attribute_info[i]->attnum == attnum) {
			strcpy(attribute_name, attribute_info[i]->attname);
		}
	}
...
exec_simple_query(const char *query_string)
{
	...
	/*CS 387 Project start*/
	Relation rel;
	const int querylen = strlen(query_string);
	char query_head[40];
	const char* add_fun_dep_compare = "ADD FUNCTIONAL DEPENDENCY IN ";
	char newquery[200];
	if (querylen > 29) {
		strncpy(query_head, query_string, 29);
		query_head[29] = '\0';
		if (strcmp(query_head, add_fun_dep_compare) == 0) {
			char relationName[65];
			int index = 29;
			int num_base = 0;
			int num_derived = 0;
			while (query_string[index] != ' ') {
				relationName[index - 29] = query_string[index];
				index++;
			}	
			relationName[index-29] = '\0';
			newquery[0] = '\0';
			char relation_metadata_query[200];
			relation_metadata_query[0] = '\0';
			strcat(relation_metadata_query, "SELECT oid, relnatts FROM pg_class WHERE relname = '");
			strcat(relation_metadata_query, relationName);
			strcat(relation_metadata_query, "';");
			strcat(newquery, relation_metadata_query);
			store_rel_metadata = 2;
			exec_simple_query(newquery);
			store_rel_metadata = 0;
			current_rel_attribute_info = (relationAttributeInfoData*) palloc(current_rel_natts*sizeof(relationAttributeInfoData));
			for (int i = 0; i < current_rel_natts; i++) {
				current_rel_attribute_info[i] = (relationAttributeInfo*) palloc(sizeof(relationAttributeInfo));
			}
			char relation_attribute_data_query[200];
			relation_attribute_data_query[0] = '\0';
			strcat(relation_attribute_data_query, "SELECT attname, attlen, attnum, attndims, attbyval FROM pg_attribute WHERE attrelid = ");
			char current_relid_str[50];
			current_relid_str[0] = '\0';
			sprintf(current_relid_str, "%d", current_relid);
			strcat(relation_attribute_data_query, current_relid_str);
			strcat(relation_attribute_data_query, " AND attnum > 0;");
			store_attribute_data = 1;
			current_att_num = 0;
			exec_simple_query(relation_attribute_data_query);
			current_att_num = 0;
			store_attribute_data = 0;

			/* 
			* Now, we have metadata corresponding to the relation
			*/
			char insert_into_fd_table_stmt[200];
			insert_into_fd_table_stmt[0] = '\0';
			strcat(insert_into_fd_table_stmt, "INSERT INTO FD_TABLE VALUES ('");
			strcat(insert_into_fd_table_stmt, relationName);
			strcat(insert_into_fd_table_stmt, "', ");
			strcat(insert_into_fd_table_stmt, current_relid_str);
			strcat(insert_into_fd_table_stmt, ", '");
			int attname_index;
			attname_index = 0;
			char attribute[65];
			...
			while (query_string[index] != ')') {
				if (query_string[index] == ',') {
					attribute[attname_index] = '\0';
					
					num_base++;
					attname_index = 0;
					int lhs_att_num = getAttNumFromCurrRelation(attribute, current_rel_natts, current_rel_attribute_info);

					char lhs_att_num_str[50];
					sprintf(lhs_att_num_str, "%d", lhs_att_num);
					strcat(insert_into_fd_table_stmt, lhs_att_num_str);
					strcat(insert_into_fd_table_stmt, ", ");
				...
				attribute[attname_index] = query_string[index];
				attname_index++;
				index++;
			}
			num_base++;
			...
			sprintf(lhs_att_num_str, "%d", lhs_att_num);
			strcat(insert_into_fd_table_stmt, lhs_att_num_str);
			strcat(insert_into_fd_table_stmt, "a', ");

			char num_base_str[50];
			sprintf(num_base_str, "%d", num_base);
			strcat(insert_into_fd_table_stmt, num_base_str);
			strcat(insert_into_fd_table_stmt, ", '");
			...
			while (query_string[index] != ')') {
				if (query_string[index] == ',') {
					...
					int rhs_att_num = getAttNumFromCurrRelation(attribute, current_rel_natts, current_rel_attribute_info);

					char rhs_att_num_str[50];
					sprintf(rhs_att_num_str, "%d", rhs_att_num);
					strcat(insert_into_fd_table_stmt, rhs_att_num_str);
					strcat(insert_into_fd_table_stmt, ", ");
			...
			char num_derived_str[50];
			sprintf(num_derived_str, "%d", num_derived);
			strcat(insert_into_fd_table_stmt, num_derived_str);
			strcat(insert_into_fd_table_stmt, ");");

			exec_simple_query(insert_into_fd_table_stmt);
			attribute[0] = '\0';

			for (int i = 0; i < current_rel_natts; i++) {
				pfree(current_rel_attribute_info[i]);
			}
			pfree(current_rel_attribute_info);
			return;
		}
	}
	const char* insert_compare = "INSERT INTO ";
	query_head[0] = '\0';
	int yes = 0;
	if (querylen > 12) {
		strncpy(query_head, query_string, 13);
		query_head[12] = '\0';
		if (strcmp(query_head, insert_compare) == 0) {
			yes = 1;
			char relationName[65];
			int index = 12;

			while (query_string[index] != ' ') {
				relationName[index - 12] = query_string[index];
				index++;
			}	
			relationName[index-12] = '\0';
			newquery[0] = '\0';

			if (strcmp(relationName, "FD_TABLE") != 0) {
				index += 9;

				getRelationMetaData(relationName);

				insert_values = (char**) palloc(current_rel_natts*sizeof(char*));
				for (int attnum = 0; attnum < current_rel_natts; attnum++) {
					insert_values[attnum] = (char*) palloc(65*sizeof(char));
				}
				int curr_attval_num = 0;
				int curr_attval_index = 0;
				while (query_string[index] != ')') {
					if (query_string[index] == ',') {
						insert_values[curr_attval_num][curr_attval_index] = '\0';
						index++;
						curr_attval_num++;
						curr_attval_index = 0;
						continue;
					}
					insert_values[curr_attval_num][curr_attval_index] = query_string[index];
					curr_attval_index++;
					index++;
				}
				insert_values[curr_attval_num][curr_attval_index] = '\0';

				/* Get the number of FDs associated with that relation */
				strcat(newquery, "SELECT COUNT(*) FROM FD_TABLE WHERE relname = '");
				strcat(newquery, relationName);
				strcat(newquery, "';");
				...
				current_rel_fd_info = (fdInfoData*) palloc(num_fds*sizeof(fdInfoData));
				for (int i = 0; i < num_fds; i++) {
					current_rel_fd_info[i] = (fdInfo*) palloc(sizeof(fdInfo));
				}
				newquery[0] = '\0';
				strcat(newquery, "SELECT * FROM FD_TABLE WHERE relname = '");
				strcat(newquery, relationName);
				strcat(newquery, "';");

				get_fds = 1; current_fd_num = 0;
				exec_simple_query(newquery);
				get_fds = 0; current_fd_num = 0;

				
				int dum = 1;

				check_fd = 1; return_only_one = 1;
				for (int i = 0; i < num_fds; i++) {
					char fd_check_query[200];
					fd_check_query[0] = '\0';
					strcat(fd_check_query, "SELECT * FROM ");
					strcat(fd_check_query, relationName);
					strcat(fd_check_query, " WHERE ");
					for (int j = 0; j < current_rel_fd_info[i]->base_count; j++) {
						char* attribute_name = getAttName(current_rel_fd_info[i]->base_atts[j], current_rel_natts, current_rel_attribute_info);
						strcat(fd_check_query, attribute_name);
						pfree(attribute_name);
						strcat(fd_check_query, " = ");
						strcat(fd_check_query, insert_values[current_rel_fd_info[i]->base_atts[j]-1]);
						strcat(fd_check_query, " AND ");
					}
					strcat(fd_check_query, "(");
					for (int j = 0; j < current_rel_fd_info[i]->derived_count; j++) {
						char* attribute_name = getAttName(current_rel_fd_info[i]->derived_atts[j], current_rel_natts, current_rel_attribute_info);
						strcat(fd_check_query, attribute_name);
						pfree(attribute_name);
						strcat(fd_check_query, " != ");
						strcat(fd_check_query, insert_values[current_rel_fd_info[i]->derived_atts[j]-1]);
						if (j < current_rel_fd_info[i]->derived_count - 1) {
							strcat(fd_check_query, " OR ");
						}
					}
					strcat(fd_check_query, ");");
					exec_simple_query(fd_check_query);

					if (check_violated) {
						check_fd = 0; return_only_one = 0; check_violated = 0;
						ereport(ERROR,
						(errcode(ERRCODE_CHECK_VIOLATION),
						errmsg("Functional dependency check violated"),
						errdetail_abort()));
					}
				}
				check_fd = 0; return_only_one = 0;
				...
				for (int i = 0; i < current_rel_natts; i++) {
					pfree(insert_values[i]);
				}
				pfree(insert_values);

				for (int i = 0; i < num_fds; i++) {
					// pfree(current_rel_fd_info[i]->base_atts);
					// pfree(current_rel_fd_info[i]->derived_atts);
					pfree(current_rel_fd_info[i]);
				}
				pfree(current_rel_fd_info);
			}
		}
	}
	/*CS 387 Project end*/
...
void getRelationMetaData (const char * relationName) {
	char newquery[200];
	newquery[0] = '\0';
	char relation_metadata_query[200];
	relation_metadata_query[0] = '\0';
